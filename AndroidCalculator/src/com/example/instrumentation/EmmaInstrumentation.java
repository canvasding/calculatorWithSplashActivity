...
import com.calculator.SplashActivity;
...
public class EmmaInstrumentation extends Instrumentation implements FinishListener {
	 ...
	 * may have been provided using {@link #setFinishListener(FinishListener)}.
	 * 
	 * It's important to note that the original Activity has not been modified. Also, the Activity
	 * must be declared in the <code>AndroidManifest.xml</code> because it is started by an intent in
	 * {@link EmmaInstrumentation#onStart()}. This turns more difficult to use other methods like
	 * using template classes.
	 * This latter method could be viable, but all Activity methods should be re-written to invoke
	 * the template parameter class corresponding methods.
	 * 
	 * @author diego
	 *
	 */
	public static class InstrumentedActivity extends SplashActivity {
		...
		@Override
		public void finish() {
			if ( LOGD ) Log.d(TAG + ".InstrumentedActivity", "finish()");
			super.finish();
			if ( mListener != null) {
	...
	public void onCreate(Bundle arguments) {
		if ( LOGD ) Log.d(TAG, "onCreate(" + arguments + ")");
	...
	private void generateCoverageReport() {
		if ( LOGD ) Log.d(TAG, "generateCoverageReport()");
